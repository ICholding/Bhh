...
import ReviewsFloater from '@/components/reviews/ReviewsFloater';
...
export default function Landing() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 via-white to-cyan-50 overflow-x-hidden">
        ...
        >
          ...
          <div className="flex flex-col gap-3 max-w-xs mx-auto">
            ...
            <div className="flex gap-2">
              <Link to={createPageUrl('EmployeeSignup')} className="flex-1">
                <Button variant="outline" className="w-full h-12 rounded-xl border-gray-200">
                  I'm an Employee
                </Button>
              </Link>
              <Link to={createPageUrl('Auth') + '?role=admin'} className="flex-1">
                <Button variant="outline" className="w-full h-12 rounded-xl border-gray-200">
                  Admin
                </Button>
              </Link>
      ...
      {/* Reviews Floater & Drawer */}
      <ReviewsFloater onClick={() => setShowReviews(true)} />
      <ReviewsDrawer isOpen={showReviews} onClose={() => setShowReviews(false)} />
    </div>
  );
}